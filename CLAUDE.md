...
- **The Law of the Realms, NORSE070–073** — the forge ships `gen/Architecture.Analyzers` (`Norse.Architecture.Analyzers` NuGet package), a standalone build-time analyzer that enforces the platform's realm dependency doctrine. Four strikes: wire format outside Midgard/Yggdrasil (NORSE070); Midgard taken as a dependency (NORSE071); cross-realm reach outside published surfaces (NORSE072); component impurity (NORSE073). All `NotConfigurable` errors — no downgrade. Delivered platform-wide via Ginnungagap scatter; jurisdiction derived entirely from assembly names (brand-blind, family-inferred). Full design and statutes: `../Glitnir/docs/Platform/specs/2026-08-03-realm-dependency-law-compiler-enforcement-design.md`.
- **NORSE ledger extended:** NORSE060-069 is Svartálfheim's block (result/parser vocabulary, identifiers); NORSE070-079 now claims architecture law (cross-realm and wire-format strikes, component purity).
