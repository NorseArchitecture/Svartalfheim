...
Each increment is spec-first: brainstorm → spec → plan → code, with explicit human greenlights at each transition. **Every plan's REQUIRED SUB-SKILL line cites `superpowers:test-driven-development` alongside `superpowers:subagent-driven-development` (or `superpowers:executing-plans`)** — the orchestration skill sequences tasks, TDD governs how each one is actually coded. The forge's existing tests already hold this line in spirit (real-code assertions, no mocks, one behavior per test); naming the skill explicitly keeps it that way as the increments get harder (the fifth increment's DST seam logic is the first one with real branching to get wrong).
