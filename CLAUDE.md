...
- **`ParseFailure` is closed** (Unspecified sentinel / Empty / Malformed / Duplicate) — adding a member is a deliberate breaking change. `Duplicate` was added 2026-08-09 (a token individually valid but repeated where each may appear once; first consumer is flags-enum array parsing in Midgard's text channels).
