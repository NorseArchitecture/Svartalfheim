...
- **The Law of the Realms, NORSE070–073 + NORSE079** — the forge ships `gen/Architecture.Analyzers` (`Norse.Architecture.Analyzers` NuGet package), a standalone build-time analyzer that enforces the platform's realm dependency doctrine. Five strikes: wire format outside Midgard/Yggdrasil (NORSE070); Midgard taken as a dependency (NORSE071); cross-realm reach outside published surfaces (NORSE072); component impurity (NORSE073); suppressing any of the above (NORSE079, the meta-strike — a `[SuppressMessage]` naming a NORSE07x rule is itself a conviction). All `NotConfigurable` errors — no downgrade, and NORSE079 convicts any `[SuppressMessage]` that names them. Delivered platform-wide via Ginnungagap scatter; jurisdiction derived entirely from assembly names (brand-blind, family-inferred). Full design and statutes: `../Glitnir/docs/Platform/specs/2026-08-03-realm-dependency-law-compiler-enforcement-design.md`.
- **NORSE ledger extended:** NORSE060-069 is Svartálfheim's block (NORSE060 — `Result<T>` reachable in a `[ServiceContract]` response); NORSE070-079 now claims architecture law (cross-realm and wire-format strikes, component purity, and the NORSE079 suppression meta-strike).
