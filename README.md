...
- **`Pii`** (`Primitives/Pii`) — masked-by-default PII scalars: `EmailAddress`, `PhoneNumber`, `PersonalName`, `BirthDate`, each an `IPiiScalar<TSelf>` over `IMaskedValue` (masked `ToString`, deliberate `WireValue`/`Normalized` egress). Paired with the NORSE061/NORSE062 compile-time retention-policy analyzer: a persisted root's PII property with no `[RetentionPolicy]` is an error, and PII reachable any way other than a direct scalar (nested, collection, array) is banned outright, no cure.
